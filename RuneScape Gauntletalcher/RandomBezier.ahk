...
RandomBezier(Xf, Yf, AR, O="" ) {
	Random, RngMove, 100, 250
    Time := RegExMatch(O,"i)T(\d+)",M)&&(M1>0)? M1: RngMove
    RO := InStr(O,"RO",0) , RD := InStr(O,"RD",0), DD := InStr(0,"DD",0)
	...
	If ( AR )	
		RNGxLess := Xf - AR
		RNGxMore := Xf + AR
		Random, Xf, RNGxLess, RNGxMore
		RNGyLess := Yf - AR
		RNGyMore := Yf + AR
		Random, Yf, RNGyLess, RNGyMore
    If ( RD )
        Xf += X0, Yf += Y0
    If ( X0 < Xf )
        sX := X0-OfL, bX := Xf+OfR
